...
(function () {
  ...
  var DICT = {
    ...
    'reducer': { cat: 'plugin', short: 'A CUSTOM reduction. Optional, and not built.',
      body: '<p>Do not read this as "reduction does not happen". <b>The reduce stage always runs</b> - it is one of the four stages, and it is what collapses your epochs into a delivered product. Twelve monthly votes becoming one annual answer is the reduce stage doing its job.</p><p><b>Where is the reducer declared, then?</b> Per layer, in each <code>aggregate</code> block - not once for the run. No shipped example names a reducer at all. This block would add a <i>plugin</i> that replaces that vocabulary with your own code.</p>',
