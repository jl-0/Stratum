...
(function () {
  ...
  var DICT = {
    ...
    'reducer': { cat: 'plugin', short: 'A CUSTOM reduction. Optional, and not built.',
      body: '<p>Do not read this as "reduction does not happen". <b>The reduce stage always runs</b> - it is one of the four stages, and it is what collapses your epochs into a delivered product. Twelve monthly votes becoming one annual answer is the reduce stage doing its job.</p><p>What this block would add is a <i>plugin</i> that replaces the built-in reduction with your own code. Omitted - as in every shipped example - the built-in reducer runs what each layer’s <code>aggregate</code> declares, which covers everything named so far.</p>',
      gotcha: 'Reduction is DECLARED, not coded: `aggregate: {method: vote, min_count: 2}` is the whole thing. A Reducer plugin is only for what that vocabulary cannot say - a joint vote across candidate layers, classify-last over per-candidate evidence - and naming one is refused at validation today.' },
    ...
    'aggregate': { cat: 'schema', short: 'How epochs collapse for this layer.',
      body: '<p><code>vote</code> for categorical, <code>median</code> / <code>percentile</code> / <code>inverse_variance</code> for continuous, <code>none</code> to carry a layer without delivering it.</p>',
